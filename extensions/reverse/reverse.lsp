...
;
; s-read-reverse(filename) will get information about file format from
; the file, so the optional parameters are only needed when the file 
; does not have a header (e.g. it is a raw file).
           ...
           (dotimes (i channels)
             (setf (aref rslt i)
                   (nyq:s-reverse-file filename time-offset fsrate dur 
                                       channels format mode bits swap i)))
           rslt)
          (t (nyq:s-reverse-file filename time-offset fsrate dur
                                 channels format mode bits swap nil)))))
    ...
    (if (> dur blockdur)
        (seq (nyq:reverse-some-samples 
              (nyq:s-read-chan filename
                    ...
                    nchans format mode bits swap chan)
              0 *reverse-blocksize*)
