...
void prod_free(snd_susp_type a_susp);
...
void prod_nn_fetch(snd_susp_type a_susp, snd_list_type snd_list)
{
    prod_susp_type susp = (prod_susp_type) a_susp;
...
void prod_ni_fetch(snd_susp_type a_susp, snd_list_type snd_list)
{
    ...
    register sample_block_values_type s1_ptr_reg;
    falloc_sample_block(out, "prod_ni_fetch");
    out_ptr = out->samples;
    snd_list->block = out;
...
void prod_nr_fetch(snd_susp_type a_susp, snd_list_type snd_list)
{
    prod_susp_type susp = (prod_susp_type) a_susp;
    int cnt = 0; /* how many samples computed */
    ...
    register sample_block_values_type s1_ptr_reg;
    falloc_sample_block(out, "prod_nr_fetch");
    out_ptr = out->samples;
    snd_list->block = out;
...
void prod_toss_fetch(snd_susp_type a_susp, snd_list_type snd_list)
    {
    prod_susp_type susp = (prod_susp_type) a_susp;
...
void prod_mark(snd_susp_type a_susp)
{
    prod_susp_type susp = (prod_susp_type) a_susp;
...
void prod_print_tree(snd_susp_type a_susp, int n)
{
    prod_susp_type susp = (prod_susp_type) a_susp;
    indent(n);
    stdputstr("s1:");
...
sound_type snd_make_prod(sound_type s1, sound_type s2)
{
    ...
    switch (interp_desc) {
      case INTERP_nn: susp->susp.fetch = prod_nn_fetch; break;
      case INTERP_ni: susp->susp.fetch = prod_ni_fetch; break;
      case INTERP_nr: susp->susp.fetch = prod_nr_fetch; break;
