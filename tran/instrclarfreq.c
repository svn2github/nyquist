...
void clarinet_freq_free(snd_susp_type a_susp);
...
{
    clarinet_freq_susp_type susp = (clarinet_freq_susp_type) a_susp;
    ...
    falloc_sample_block(out, "clarinet_freq_ns_fetch");
    out_ptr = out->samples;
    snd_list->block = out;
...
void clarinet_freq_mark(snd_susp_type a_susp)
{
    clarinet_freq_susp_type susp = (clarinet_freq_susp_type) a_susp;
...
void clarinet_freq_print_tree(snd_susp_type a_susp, int n)
{
    clarinet_freq_susp_type susp = (clarinet_freq_susp_type) a_susp;
    indent(n);
...
sound_type snd_make_clarinet_freq(double freq1, sound_type breath_env, sound_type freq_env, rate_type sr)
{
    ...
    susp->susp.fetch = clarinet_freq_ns_fetch;
...
sound_type snd_clarinet_freq(double freq1, sound_type breath_env, sound_type freq_env, rate_type sr)
{
    sound_type breath_env_copy = sound_copy(breath_env);
    sound_type freq_env_copy = sound_copy(freq_env);
    return snd_make_clarinet_freq(freq1, breath_env_copy, freq_env_copy, sr);
}
