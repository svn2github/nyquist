...
void tapv_free(snd_susp_type a_susp);
...
{
    tapv_susp_type susp = (tapv_susp_type) a_susp;
    ...
    falloc_sample_block(out, "tapv_nn_fetch");
    out_ptr = out->samples;
    snd_list->block = out;
...
void tapv_ni_fetch(snd_susp_type a_susp, snd_list_type snd_list)
{
    tapv_susp_type susp = (tapv_susp_type) a_susp;
    int cnt = 0; /* how many samples computed */
    ...
    register sample_block_values_type s1_ptr_reg;
    falloc_sample_block(out, "tapv_ni_fetch");
    out_ptr = out->samples;
    snd_list->block = out;
...
void tapv_nr_fetch(snd_susp_type a_susp, snd_list_type snd_list)
{
    tapv_susp_type susp = (tapv_susp_type) a_susp;
    int cnt = 0; /* how many samples computed */
    ...
    register sample_type * buffer_reg;
    register sample_block_values_type s1_ptr_reg;
    falloc_sample_block(out, "tapv_nr_fetch");
    out_ptr = out->samples;
    snd_list->block = out;
...
void tapv_si_fetch(snd_susp_type a_susp, snd_list_type snd_list)
{
    tapv_susp_type susp = (tapv_susp_type) a_susp;
    int cnt = 0; /* how many samples computed */
    sample_type vardelay_x2_sample;
...
void tapv_toss_fetch(snd_susp_type a_susp, snd_list_type snd_list)
    ...
    susp->s1_ptr += n;
    susp_took(s1_cnt, n);
    n = ROUNDBIG((final_time - susp->vardelay->t0) * susp->vardelay->sr -
...
void tapv_mark(snd_susp_type a_susp)
{
    tapv_susp_type susp = (tapv_susp_type) a_susp;
...
void tapv_print_tree(snd_susp_type a_susp, int n)
{
    tapv_susp_type susp = (tapv_susp_type) a_susp;
    indent(n);
    stdputstr("s1:");
...
sound_type snd_make_tapv(sound_type s1, double offset, sound_type vardelay, double maxdelay)
{
    ...
    switch (interp_desc) {
      ...
      case INTERP_nn: susp->susp.fetch = tapv_nn_fetch; break;
      case INTERP_ni: susp->susp.fetch = tapv_ni_fetch; break;
      case INTERP_nr: susp->susp.fetch = tapv_nr_fetch; break;
