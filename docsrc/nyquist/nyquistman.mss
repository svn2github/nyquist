...
original "UPIC" editor. Azarakhsh Keipour wrote the original extension manager.
...
below).  Currently, tables are limited to 100,000,000 samples.  This limit is the compile-time constant @code(max_table_len) set in @code(sound.h). A wavetable is a list of the form
@begin(display)
...
Access this element of @code(*rslt*) by calling @code[snd-read-flags(*rslt*)].
...
functions). 
Access this element of @code(*rslt*) by calling @code[snd-byte-offset(*rslt*)].
...
@begin(fndefs)
@codef{next(@pragma(defn)@index(next)@i(pattern-object) [, #t])} @c{[sal]}@*
@altdef{@code{(next @i(pattern-object) [t])} @c{[lisp]}}@\Returns the next element from a pattern generator object. If the optional second argument is true (default value is false), then an entire period is returned as a list.
@end(fndefs) 
...
period lengths match the period lengths from @i(sub-pattern). 
If @i(maximum) (a pattern or a number) is specified, and the 
running sum exceeds @i(maximum), the running sum is reset 
to @i(maximum). If @i(minimum) (a pattern or a number) is 
specified, and the running sum falls below @i(minimum), the 
running sum is reset to @i(minimum). If @i(minimum) is greater 
than @i(maximum), the running sum will be set to one of the two 
values. Minimum and maximum patterns are reevaluated every cycle. 
Note that this is similar in name but not in function to @code(make-accumulation).
@end(fndefs)
...
be evaluated, and the output periods match the periods of the expression pattern.
...
@i(expr) to generate each item. If @i(expr) is a pattern, each item is generated by getting 
the next item from @i(expr) and evaluating it.
@end(fndefs)
...
is returned. If @i(keyword) is @code(:pitch) and a corresponding parameter value
is a list, each element of the list is incremented by @i(amount). This special
case is in keeping with the convention of @code(timed-seq) in which score
events with lists for the @code(:pitch) attribute are expanded into "chords"
by instantiating an event for each element (pitch) in the list (chord).
...
@altdef{@code{(score-write @i(score) @i(filename) [@i(programs) @i(absolute)]} @c{[lisp]}}@\Write an Adagio format file to @i(filename) with notes in @i(score), using absolute times if @i(absolute) is true, otherwise write relative times (the default). See Chapter @ref(adagio-chap) for details on Adagio, a text-based score language. See @code(score-write-smf) for details on MIDI program changes.
@end(fndefs)
